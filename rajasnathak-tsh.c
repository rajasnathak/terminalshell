...
void sigint_handler(int sig)
{
  pid_t fg_pid=fgpid(jobs);
         if(fg_pid==0)
         {
 		printf("no foreground jobs currently\n");
    return;
 	        }
         else if(fg_pid>0)
         {
 ...
 */
void sigtstp_handler(int sig)
{   //get pid of the fg job
    pid_t fpid = fgpid(jobs);
     //If there is a fg job
     if(fpid > 0)
     {
       //Send SIGTSTP to all processes in fg
       kill(-fpid, SIGTSTP);
     }
     return;
  }
